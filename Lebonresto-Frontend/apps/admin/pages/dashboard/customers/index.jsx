...
export default function Customers() {
    const columns = [
            ...
            render: (row) => (
                <div className="d-flex align-items-center">
                    <div className="avatar avatar-md me-3">
                        <div className="avatar-img rounded-circle bg-success-subtle text-success d-flex align-items-center justify-content-center fw-bold border">
                            C{row.id}
                        </div>
                    </div>
                    <div>
                        <h6 className="mb-0 fw-semibold text-dark">Customer Name {row.id}</h6>
                        <small className="text-muted">ID: #CUST-{1000 + row.id}</small>
                    </div>
                </div>
            )
        },
        {
            ...
            render: (row) => (
                <div className="d-flex flex-column">
                    <span className="text-sm text-dark"><i className="bi bi-envelope me-1 text-muted"></i> customer{row.id}@example.com</span>
        ...
        },
        {
            ...
            accessor: 'bookings',
            render: (row) => (
                <span className="admin-badge admin-badge-secondary">{row.id * 2} Reservations</span>
            )
        },
        {
            header: 'Status',
            ...
            render: (row) => (
                <div className="dropdown">
                    ...
                    <ul className="dropdown-menu dropdown-menu-end border-0 shadow-sm">
                        <li><Link href={`/dashboard/customers/${row.id}`} className="dropdown-item py-2"><FiEye className="me-2" /> View Details</Link></li>
    ...
    return (
        <div className="container-fluid">
            <AdminPageHeader
                title="Customers"
                subtitle="Manage registered customers and their booking history."
            />
