...
import { loginCustomer, loginOwner, loginAdmin } from '../../store/slices/authSlice';
...
export default function LoginForm() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formState.emailOrUsername || !formState.password) {
            setLocalError('Please fill in all fields');
            return;
        }

        try {
            // Defaulting to customer login for this general interface
            // Ideally we could detect role or have tabs, but per req we call login endpoint
            const resultAction = await dispatch(loginCustomer({
            ...
            if (loginCustomer.fulfilled.match(resultAction)) {
                ...
                // If customer login fails, try owner login transparently? 
                // Or just show error. For now, showing error.
                // The provided Redux slice has separate actions.
                // NOTE: The 'loginCustomer' thunk expects 'email', but our form uses 'emailOrUsername'.
                // If backend supports username, we map it. 
                // Assuming backend expects 'email' key even for username or we adjust payload.
    ...
    return (
        <form onSubmit={handleSubmit} className="w-100">
            ...
            <div className="form-floating mb-4 position-relative">
                ...
                <div className="text-end mt-2">
                    <Link href="/forgot-password" className="text-decoration-none small text-muted hover-primary">
                        First connection / Forgot password?
                    </Link>
                </div>
            </div>
            ...
            <div className="text-center">
                <span className="text-muted me-2">Not a member yet?</span>
